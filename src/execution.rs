...
impl ExecutionEngine {
        ...
        match result {
            ...
            Ok((yes_filled, no_filled, yes_cost, no_cost, yes_order_id, no_order_id)) => {
                ...
                if matched > 0 {
                    ...
                    let (platform1, side1, platform2, side2) = ("polymarket", "yes", "polymarket", "no");
    ...
    async fn execute_both_legs_async(
        ...
        // Poly-only: Buy YES + Buy NO simultaneously
    ...
    /// Extract results from Poly-only execution
    ...
    async fn auto_close_background(
        ...
        let log_close_pnl = |closed: i64, proceeds: i64| {
            if closed > 0 {
                ...
                warn!("[EXEC] ⚠️ Failed to close Poly excess - 0 filled");
            }
        };

        // Poly-only: close YES or NO depending on which side has excess
